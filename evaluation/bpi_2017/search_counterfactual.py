...
trace_backward = process_execution_cfg.get("trace_backward")
...
print(f"Found {len(viewpoint_ids)} viewpoint IDs with label {viewpoint_label}.")
...
for viewpoint_id in viewpoint_ids:
    ...
    target_process_execution = ocel_nx.subgraph(events | objects).copy()
    ...
    tree_search = TreeSearchCounterFactual(
        ...
        log_file=f"logs/{viewpoint_id}.log",
    )
