...
# Determine viewpoint object ids
viewpoint_ids = []
if counterfactual_cfg.get("viewpoint_id") is not None:
    ...
    for viewpoint_id, label in labels_data["viewpoint_object_labels"].items():
...
for viewpoint_id in viewpoint_ids:
    ...
    event_insertion_actions = build_event_insertion_actions(
        ...
        viewpoint_ids=event_insertion_viewpoint_ids,
    )
    ...
    tree_search = TreeSearchCounterFactual(
        ...
        log_file=f"logs/road_traffic_fine_management-{viewpoint_id}.log",
    )
    ...
    for action_set in sorted_action_sets:
        ...
        for other_id in viewpoint_ids:
            events = set([e for e, _ in ocel_nx.in_edges(other_id)])
            nodes = events | {other_id}
