...
def _compute_proximity(
    ...
    proximity_score = dist_x + dist_a

    return {
        "dist_x": dist_x.item(),
        "dist_a": dist_a.item(),
        "proximity": proximity_score.item(),
    }
...
def generate_graphcfe_counterfactual_with_proximity(
    graphcfe_model: torch.nn.Module,
    graph: HeteroData | Data,
    target_class: int,
    dataset_stats: GraphCFEDatasetStats,
    ...
    Tuple[List[GraphEdit], dict]
        ...
        - proximity: dict with keys 'dist_x', 'dist_a', 'proximity'
    """
    explainer = GraphCFEExplainer(
        graphcfe_model=graphcfe_model,
        dataset_stats=dataset_stats,
    ...
    return explainer.explain_with_proximity(graph, target_class, n_samples=n_samples)
